...
class MultiHeadedAttentionRelative(MultiHeadedAttention):
    ...
    def forward(self, key, value, query, mask=None, enc_attn_cache=None, self_attn_cache=None):
        ...
        # 这个是Q: [query_len, batch_size*head_count, dim]
        temp_query_up = query_up.view(batch_size * head_count, query_len, -1).transpose(0, 1)
        # : [query_len, query_len, dim]
        relative_key = self.relative_embedding_keys(query_len)
        # rp_scores: [query_len, batch_size*head_count, query_len]
        rp_scores = torch.matmul(temp_query_up, relative_key.transpose(-1, -2))
        rp_scores = rp_scores.transpose(0, 1).view(batch_size, head_count, query_len, query_len)
        scores = scores + rp_scores

        # 3) Apply attention dropout and compute context vectors.
        attn = self.sm(scores)
        drop_attn = self.dropout(attn)  # [batch_size, head_count, query_len, query_len]
        # value_up: [batch_size, num_head, value_len, dim_head]
        # context: [batch_size, num_head, query_len, dim_head]
        context = torch.matmul(drop_attn, value_up)  # key_len = value_len
        # [query_len, batch_size* head_count, dim]
        temp_drop_attn = drop_attn.view(batch_size * head_count, query_len, key_len).transpose(0, 1)
        # [value_len, value_len, dim]
        relative_value = self.relative_embedding_values(value_len)
        # [query_len, batch_size*head_count, size]
        rp_context = torch.matmul(temp_drop_attn, relative_value)
        rp_context = rp_context.transpose(0, 1).view(batch_size, head_count, query_len, value_len)
